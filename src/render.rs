...
use crate::arguments::{Arguments, Decorations, HighlightMode};
use crate::components::{
    add_window_buttons, add_window_title, embed_font, get_bounding_box, get_shadow, get_text_width,
};
use crate::util::{rgb_to_hex, rgb_to_yuv, yuv_to_rgb};

const WINDOW_BAR_HEIGHT: f32 = 30.0;
const TEXT_PADDING_X: f32 = 20.0;
const TEXT_PADDING_Y: f32 = 20.0;
...
pub fn render(
    ...
    let window_bar_exists = if let Some(window_title) = &args.window_title {
        ...
        false
    } || args.window_decorations != Decorations::None;
    let mut current_y = TEXT_PADDING_Y
    ...
    for line_number in selected_lines_iter {
        ...
        line_content = format!(
            "<tspan x=\"{}\" y=\"{}\">{}</tspan>",
            TEXT_PADDING_X, current_y, line_content
        );
    ...
    match args.highlight_mode {
        ...
        HighlightMode::AlignRight => {
            highlight_rect = highlight_rect
                ...
                .set("x", TEXT_PADDING_X + line_number_offset);
            defs = defs.add(highlight_rect);
        }
        _ => {}
    }
    ...
    if let Some(window_title) = &args.window_title {
        let header_text = add_window_title(window_title, &fonts_str, default_text_color, current_x);
