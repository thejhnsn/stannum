...
Create vector images of your source code! It's like a screenshot, but better. Stannum is an alternative
to [Carbon](https://carbon.now.sh) and [Silicon](https://github.com/Aloxaf/silicon) that outputs a vector image instead
of a raster image.
...
```bash
git clone https://github.com/thejhnsn/stannum
cd stannum
cargo install --path .
```
...
```bash
stannum [OPTIONS] <input>
```

## Themes

Stannum uses [TextMate](https://macromates.com/manual/en/language_grammars) themes (`.tmTheme` files) for syntax
highlighting.
